...
const DashboardView = ({ onNew, isDark, toggleDark }) => {
  ...
  const StatusBadge = ({ chantier }) => {
    ...
    if (status === 'SENT') {
      ...
      return <div className="flex items-center gap-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-1 rounded-full text-xs font-bold"><Lock size={12} /><span>Envoyé {dateStr}</span></div>;
    }
    if (status === 'ERROR') return <div className="flex items-center gap-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 px-2 py-1 rounded-full text-xs font-bold"><AlertCircle size={12} /> Erreur</div>;
      ...
      <main className="flex-1 p-4 max-w-5xl mx-auto w-full">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-bold text-slate-700 dark:text-slate-300">
            {filter === 'all' ? 'Tous les dossiers' : filter === 'draft' ? 'Dossiers en cours' : 'Dossiers envoyés'}
          </h2>
...
const ChantierDetailView = () => {
  ...
  const handleSendSuccess = () => {
    ...
    setToast({ message: 'Dossier envoyé et verrouillé !', type: 'success' });
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-20 max-w-3xl mx-auto w-full">
        ...
        <div className="space-y-4">
          ...
          <div className={`bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-200 dark:border-slate-800 ${lockedInput}`}>
            <SelectToggle label="Type" value={p.type} onChange={v => up('type', v)} disabled={isReadOnly} options={[{ label: 'Fenêtre', value: 'FENETRE' }, { label: 'Baie', value: 'BAIE_COULISSANTE' }, { label: 'Porte Entrée', value: 'PORTE_ENTREE' }, { label: 'Porte Service', value: 'PORTE_SERVICE' }, { label: 'Volet', value: 'VOLET_ROULANT' }, { label: 'Autre', value: 'AUTRE' }]} />
          ...
          {['FENETRE', 'BAIE_COULISSANTE', 'PORTE_ENTREE', 'PORTE_SERVICE'].includes(p.type) && (
            <div className={`bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-200 dark:border-slate-800 ${lockedInput}`}>
              ...
              {p.matiere && <SelectToggle id="field-profil" label="Profil" value={p.profil} onChange={v => up('profil', v)} error={isE('profil')} disabled={isReadOnly} options={p.matiere === 'PVC' ? [{ label: 'Neuf', value: 'NEUF' }, { label: 'Rénov 40', value: 'RENO_40' }, { label: 'Rénov 60', value: 'RENO_60' }, { label: 'ISO', value: 'ISO' }] : [{ label: 'Neuf', value: 'NEUF' }, { label: 'Rénov', value: 'RENO' }]} />}
          ...
          {(p.type === 'FENETRE' || p.type === 'BAIE_COULISSANTE') && (
            <div id="field-vitrageFlags" className={`bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border ${isE('vitrageFlags') ? 'border-red-500 ring-1 ring-red-500' : 'border-slate-200 dark:border-slate-800'} ${lockedInput}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[['standard', '4/20/4'], ['g200', 'G200'], ['feuillete1f', 'SP10 1F'], ['feuillete2f', 'SP10 2F']].map(([k, l]) => (
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-4">
                  {(p.type.includes('FENETRE') || p.type.includes('PORTE')) && (
                    <div className="space-y-3">
                      ...
                      {p.type !== 'PORTE_ENTREE' && (
                        <label className={`flex items-center justify-between ${isReadOnly ? 'cursor-not-allowed' : ''}`}>
                          <span className="dark:text-white">Serrure à clé</span>
                          <input type="checkbox" className="w-5 h-5 accent-brand-600" checked={!!p.serrureCle} onChange={e => up('serrureCle', e.target.checked)} disabled={isReadOnly} />
                        </label>
                      )}
                      {p.type === 'PORTE_ENTREE' && (
                        <label className={`flex items-center justify-between ${isReadOnly ? 'cursor-not-allowed' : ''}`}>
 ...
 * @param {Function} onStatusChange - Callback pour mettre à jour le status
...
async function sendToGoogleSheets(chantier, products, htmlReport, onStatusChange) {
    ...
    produits: products.map(p => ({
      ...
      vitrage: p.vitrageFlags ? Object.entries(p.vitrageFlags)
        ...
        .map(([k]) => {
          ...
          if (k === 'feuillete1f') return 'SP10 1F';
          if (k === 'feuillete2f') return 'SP10 2F';
          return '';
        })
        .filter(Boolean)
        .join(', ') : '',
  ...
  try {
    ...
    Logger.error("❌ Échec envoi métrage", { error: errorMessage, client: chantier.client });
 ...
 * Checklist de vérification + Alerte verrouillage
 */
const ConfirmSendModal = ({ chantier, products, incompleteCount, onClose, onStatusChange, onSuccess, onError }) => {
  ...
  const handleSend = async () => {
    ...
    if (result.success) { onSuccess?.(); } else { onError?.(result.error); }
  };

  return (
    <Modal isOpen={true} onClose={onClose} title="Êtes-vous sûr ?" size="md">
      {/* Checklist */}
      ...
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 mb-4 flex items-start gap-3">
        ...
        <div>
          <p className="font-bold text-red-700 dark:text-red-300 text-sm">Attention</p>
          <p className="text-red-600 dark:text-red-400 text-sm">Le dossier sera verrouillé après envoi.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 mb-6">
        <div className="flex justify-between text-sm mb-2">
          ...
          <span className="font-bold dark:text-white">{products.reduce((a, p) => a + (p.quantity || 1), 0)}</span>
        </div>
        {incompleteCount > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-amber-600 dark:text-amber-400 flex items-center gap-1"><AlertTriangle size={14} /> Incomplets</span>
            <span className="font-bold text-amber-600 dark:text-amber-400">{incompleteCount}</span>
          </div>
        )}
      </div>
      ...
      {/* Actions */}
      <div className="space-y-3">
        <Button onClick={handleSend} disabled={!allChecked || isSending} className="w-full py-4" icon={isSending ? null : Send}>
          {isSending ? <span className="flex items-center justify-center gap-3"><Spinner size={20} />Envoi en cours...</span> : "Confirmer l'envoi"}
        </Button>
        <Button onClick={onClose} variant="ghost" className="w-full" disabled={isSending}>Annuler</Button>
      </div>
    </Modal>
  );
};
...
/**
 * Modal de déverrouillage sécurisé
 * L'utilisateur doit saisir "MODIFIER" pour déverrouiller
 */
