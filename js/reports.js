...
export const generateReportHTML = (c, prods, dt) => {
    ...
    prods.forEach(p => {
        ...
        if (['FENETRE', 'BAIE_COULISSANTE', 'PORTE_ENTREE', 'PORTE_SERVICE'].includes(p.type)) {
            ...
            if (p.type === 'FENETRE') { add('Oscillo-Battant', p.oscilloBattant); add('Grille Vent.', p.grilleVentilation); if (p.poigneeHauteur === 'AUTRE' && p.poigneeHauteurMm) add('Hauteur Poignée', `${p.poigneeHauteurMm} mm`); add('Serrure Clé', p.serrureCle); add('Ouv. Ext.', p.ouvertureExterieure) }
