import { ValidationService } from "./utils.js";
...
export const generateReportHTML = (c, prods, dt) => {
    ...
    let h = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Relevé ${c.client}</title><style>${css}</style></head><body onload="window.print()"><div class="header"><div class="client-box"><h1 style="margin:0 0 10px 0;color:#2563EB;">${c.client}</h1><div>📍 ${c.adresse}</div><div>📞 ${c.telephone} ${c.email ? ` • ✉️ ${c.email}` : ''}</div></div><div class="meta-box"><div class="stat-badge" style="background:${bS}">${bT}</div><br><strong>Date:</strong> ${fD} à ${fT}<br><strong>Produits:</strong> ${prods.reduce((a, b) => a + (b.quantity || 1), 0)}<br></div></div>${c.typeContrat === 'SOUS_TRAITANCE' ? `<div class="final-client-box"><strong>👷 CLIENT FINAL (CHANTIER) :</strong> ${c.clientFinal} - ${c.adresseFinale}</div>` : ''}`;
    prods.forEach(p => {
        let r = ''; const add = (l, v) => { if (v !== undefined && v !== null && v !== false && v !== '') r += `<tr><td class="label">${l}</td><td class="value">${v === true ? 'OUI' : v}</td></tr>` };
        if (p.room) add('Localisation', `<strong>${p.room}</strong>`); add('Quantité', `<strong>x ${p.quantity || 1}</strong>`);
        ...
        if (ValidationService.isMeasureSuspicious(p.largeurMm, p.monobloc) || ValidationService.isMeasureSuspicious(p.hauteurMm, p.monobloc)) r += `<tr><td class="label">⚠️ Attention</td><td class="value alert-text">Cote < 300mm</td></tr>`;
        if (['FENETRE', 'PORTE_FENETRE', 'BAIE_COULISSANTE', 'PORTE_ENTREE', 'PORTE_SERVICE'].includes(p.type)) {
            ...
            if (p.vitrageFlags) { const v = p.vitrageFlags, va = []; if (v.standard) va.push('4/20/4'); if (v.g200) va.push('G200'); if (v.feuillete1f) va.push('Feuilleté (1F)'); if (v.feuillete2f) va.push('Feuilleté (2F)'); if (va.length) add('Vitrage', va.join(', ')) }
            if (p.type === 'FENETRE' || p.type === 'PORTE_FENETRE') { add('Oscillo-Battant', p.oscilloBattant); add('Grille Vent.', p.grilleVentilation); if (p.poigneeHauteur === 'AUTRE' && p.poigneeHauteurMm) add('Hauteur Poignée', `${p.poigneeHauteurMm} mm`); add('Poignée Clé', p.poigneeCle); add('Ouv. Ext.', p.ouvertureExterieure); if (p.type === 'FENETRE' && p.pieceAppuiDescription) add('Pièce Appui', p.pieceAppuiDescription); if (p.type === 'PORTE_FENETRE') { add('Soubassement', p.soubassement ? `${p.soubassementHauteurMm}mm` : false); add('Serrure', p.serrure); add('Seuil PMR', p.seuilPMR); } }
            if (p.type === 'PORTE_ENTREE') { add('Remplissage', p.remplissage === 'PANNEAU_DECORATIF_ALU' ? 'Panneau Déco Alu' : p.remplissage); add('Seuil PMR', 'OUI'); if (p.tierceImposte) add('Tierce/Imposte', 'OUI'); if (p.tierce) add('Tierce', `Passage: ${p.cotePassageMm}mm`); add('Ouv. Ext.', p.ouvertureExterieure) }
            if (p.type === 'PORTE_SERVICE') add('Seuil PMR', 'OUI');
        }
        if (p.type === 'VOLET_ROULANT') { add('Manoeuvre', p.manoeuvre); if (p.sortieCable) add('Sortie Câble', p.sortieCable); if (p.manoeuvre !== 'FILAIRE' && p.boxDomotique) add('Box Domo', 'OUI'); if (p.coupleMoteur) add('Couple', `${p.coupleMoteur}Nm`); if (p.pose) add('Pose', p.pose); if (p.monobloc) add('Monobloc', `OUI (Coffre: ${p.coffreADeduireMm}mm)`); if (p.lieAProduitId) add('Lié à', `#${prods.find(x => x.id === p.lieAProduitId)?.index || '?'}`) }
        ...
        h += `<div class="product-card" style="${!p.isValid ? 'border-color:#ef4444;border-width:2px' : ''}"><div class="product-header"><span>#${String(p.index).padStart(2, '0')} - ${p.type}</span>${!p.isValid ? '<span style="color:#ef4444">⚠️ INCOMPLET</span>' : ''}</div><div class="product-body"><div class="specs"><table class="spec-table">${r}</table>${nt}</div><div class="visuals">${dr}${im}</div></div></div>`;
    });
